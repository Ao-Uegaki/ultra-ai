...
比較の読み方: **treatment vs control = 総合効果**(harness + 思考量)、**treatment vs control-xhigh = 仕組みのみ**
(思考量を揃えた上での harness 効果)。集計列は `ab_report.py` が両方を出す。

採点は **transcript から決定論**(headless で Stop hook 発火は未保証だが transcript は必ず書かれる)。
`metrics.py`(cost-weighted)と `compare.py` の指標定義を流用する。
...
指標: **pass_rate / pass@k**(higher-is-better)、**cost_weighted / turns_main / peak_main_context**
(lower-is-better)、**cache_read_total**(higher-is-better)。
...
- **effort の交絡**: treatment は effort=xhigh 固定。control は既定 effort なので、treatment vs control の差の一部は
  「harness」でなく「思考量」。これを切り分けるための `control-xhigh` アーム(素 config + `--effort xhigh`)があり、
  **treatment vs control-xhigh = harness のみ**を見られる。
