...
有効化(active)すると 全プロジェクト共通の学習した約束ごと に格上げ**され、全プロジェクトの SessionStart で毎回まったく同じ文章で文脈へ読み込まれる
(`UA_GLOBAL_INSTINCTS=0` で無効)。これは別リポでの**単発訂正でも格上げを許す**唯一のルールベースの
...
2. 内容を**日本語で要約**して報告(global 件数 / もうすぐ格上げされそうな 1 票の項目 / 出どころ repo 数)。
...
- `UA_GLOBAL_INSTINCTS=0` で global の記録・読み込み・格上げをすべて停止(repo ごとの二値挙動に戻る)。`ultra-ai-safe` で全 hook 無効。
- 格上げは自動(2+ repo で有効化)が基本。手動 pin は例外措置。
